...
class WCT_Admin_Page {
	...
	public static function register(): void {
		add_options_page(
			__( 'WP Client Tour', 'kingsbury-client-tour' ),
			__( 'WP Client Tour', 'kingsbury-client-tour' ),
			'manage_options',
			'wp-client-tour',
	...
	public static function render_page(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to access this page.', 'kingsbury-client-tour' ) );
		}
		...
		<div class="wrap">
			<h1>
			<?php esc_html_e( 'WP Client Tour', 'kingsbury-client-tour' ); ?>
			<span style="font-size:13px;font-weight:400;color:#646970;margin-left:10px;">v<?php echo esc_html( WCT_VERSION ); ?></span>
		</h1>

			<?php self::render_notices(); ?>

			<h2><?php esc_html_e( 'Tours', 'kingsbury-client-tour' ); ?></h2>

			<?php if ( empty( $tours ) ) : ?>
				<p><?php esc_html_e( 'No tours found. Add JSON tour files to the tours/ directory inside the plugin folder.', 'kingsbury-client-tour' ); ?></p>
			<?php else : ?>
				<table class="widefat striped">
					<thead>
						<tr>
							<th><?php esc_html_e( 'Tour ID', 'kingsbury-client-tour' ); ?></th>
							<th><?php esc_html_e( 'Label', 'kingsbury-client-tour' ); ?></th>
							<th><?php esc_html_e( 'Target Page', 'kingsbury-client-tour' ); ?></th>
							<th><?php esc_html_e( 'Roles', 'kingsbury-client-tour' ); ?></th>
							<th><?php esc_html_e( 'Steps', 'kingsbury-client-tour' ); ?></th>
							<th><?php esc_html_e( 'Trigger', 'kingsbury-client-tour' ); ?></th>
							<th><?php esc_html_e( 'Created', 'kingsbury-client-tour' ); ?></th>
						</tr>
					</thead>
					<tbody>
			...
			<h2><?php esc_html_e( 'Settings', 'kingsbury-client-tour' ); ?></h2>
			<form method="post">
				<?php wp_nonce_field( 'wct_save_settings', self::NONCE_FIELD ); ?>
				<input type="hidden" name="wct_action" value="save_settings">
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Test Mode', 'kingsbury-client-tour' ); ?></th>
						<td>
							<label>
								<input type="checkbox" name="wct_test_mode" value="1"<?php checked( $test_mode ); ?>>
								<?php esc_html_e( 'Make all auto_once tours replay on every page load (useful during development)', 'kingsbury-client-tour' ); ?>
							</label>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Dashboard Widget', 'kingsbury-client-tour' ); ?></th>
						<td>
							<label>
								<input type="checkbox" name="wct_dashboard_widget" value="1"<?php checked( $widget_enabled ); ?>>
								<?php esc_html_e( 'Show a tour launcher on the WordPress Dashboard for users who have eligible tours', 'kingsbury-client-tour' ); ?>
							</label>
						</td>
					</tr>
				</table>
				<p class="submit">
					<button type="submit" class="button button-primary">
						<?php esc_html_e( 'Save Settings', 'kingsbury-client-tour' ); ?>
					</button>
				</p>
			</form>

			<h2><?php esc_html_e( 'Updates', 'kingsbury-client-tour' ); ?></h2>
			<?php
			...
			if ( isset( $_GET['wct_refreshed'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				?>
				<div class="notice notice-success inline"><p><?php esc_html_e( 'Update cache refreshed.', 'kingsbury-client-tour' ); ?></p></div>
			<?php endif; ?>
			<p>
				<?php
				if ( $latest ) {
					if ( version_compare( $latest, WCT_VERSION, '>' ) ) {
						printf(
							...
							esc_html__( 'Version %s is available on GitHub.', 'kingsbury-client-tour' ),
							esc_html( $latest )
						);
					} else {
						esc_html_e( 'You are running the latest version.', 'kingsbury-client-tour' );
					}
				} else {
					esc_html_e( 'Update status unavailable (GitHub API unreachable).', 'kingsbury-client-tour' );
				}
				?>
			</p>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<button type="submit" class="button button-secondary">
					<?php esc_html_e( 'Check for Updates', 'kingsbury-client-tour' ); ?>
				</button>
			</form>

			<h2><?php esc_html_e( 'Reset Completion Data', 'kingsbury-client-tour' ); ?></h2>

			<form method="post" style="margin-bottom: 24px;">
				<?php wp_nonce_field( 'wct_reset_all', self::NONCE_FIELD ); ?>
				<input type="hidden" name="wct_action" value="reset_all">
				<p><?php esc_html_e( 'Clear completion data for every user. All tours marked as seen will appear again.', 'kingsbury-client-tour' ); ?></p>
				<button type="submit" class="button button-secondary"
					onclick="return confirm('<?php echo esc_js( __( 'Reset completion data for ALL users?', 'kingsbury-client-tour' ) ); ?>')">
					<?php esc_html_e( 'Reset All Users', 'kingsbury-client-tour' ); ?>
				</button>
			</form>
			...
			<form method="post">
				...
				<p><?php esc_html_e( 'Clear completion data for a specific user.', 'kingsbury-client-tour' ); ?></p>
				<label for="wct_username"><?php esc_html_e( 'Username:', 'kingsbury-client-tour' ); ?></label>
				<input type="text" id="wct_username" name="wct_username" class="regular-text" autocomplete="off">
				<button type="submit" class="button button-secondary">
					<?php esc_html_e( 'Reset User', 'kingsbury-client-tour' ); ?>
				</button>
			</form>
		</div>
		<?php
	...
	private static function handle_actions(): void {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to perform this action.', 'kingsbury-client-tour' ) );
		}
		...
		if ( ! isset( $_POST[ self::NONCE_FIELD ] )
			...
			wp_die( esc_html__( 'Security check failed.', 'kingsbury-client-tour' ) );
		}
		...
		switch ( $action ) {
			case 'save_settings':
				...
				self::add_notice( __( 'Settings saved.', 'kingsbury-client-tour' ) );
				break;

			case 'reset_all':
				self::reset_all_users();
				self::add_notice( __( 'Completion data cleared for all users.', 'kingsbury-client-tour' ) );
				break;
			...
			case 'reset_user':
				$username = isset( $_POST['wct_username'] )
					...
					: '';
				if ( '' === $username ) {
					self::add_notice( __( 'Please enter a username.', 'kingsbury-client-tour' ), 'error' );
					break;
				}
				...
				self::add_notice( __( 'Reset complete.', 'kingsbury-client-tour' ) );
				break;
		}
	}
