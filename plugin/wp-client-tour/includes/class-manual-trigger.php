...
class WCT_Manual_Trigger {
	...
	public static function render_shortcode( array $atts ): string {
		$atts = shortcode_atts(
			array(
				'tour'  => '',
				'label' => __( 'Start tour', 'kingsbury-client-tour' ),
			),
			$atts,
			'wct_launch'
		);
	...
	public static function register_admin_bar( WP_Admin_Bar $wp_admin_bar ): void {
		...
		$wp_admin_bar->add_node(
			array(
				'id'    => 'wct-tours',
				'title' => __( 'Tours', 'kingsbury-client-tour' ),
				'href'  => false,
			)
		);
		...
		foreach ( $manual_tours as $tour ) {
			...
			$title       = $is_done
				...
				? sprintf( __( '%s (Replay)', 'kingsbury-client-tour' ), esc_html( $label ) )
				: esc_html( $label );
			$url         = self::get_launch_url( $tour['id'] );
			$aria_label  = sprintf(
				/* translators: %s: tour label */
				__( 'Launch %s tour', 'kingsbury-client-tour' ),
				esc_attr( $label )
			);
