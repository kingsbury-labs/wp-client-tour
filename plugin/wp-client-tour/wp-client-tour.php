...
require_once WCT_PLUGIN_DIR . 'includes/class-manual-trigger.php';
...
add_action( 'plugins_loaded', array( 'WCT_Manual_Trigger', 'init' ) );

/**
 * Return the admin URL that launches a specific tour by ID.
 * Returns false if the tour ID does not exist.
 *
 * @param string $tour_id Tour ID (matches the "id" field in tour JSON).
 * @return string|false
 */
function wct_tour_launch_url( string $tour_id ) {
	return WCT_Manual_Trigger::get_launch_url( $tour_id );
}
